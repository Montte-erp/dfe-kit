...
if (command.length === 0) {
  console.error("Uso: bun tooling/static-checks/check-dts-warnings.ts -- <comando de build>");
  process.exit(1);
}
...
if (findings.length > 0) {
  console.error("DTS warning check failed. Build gerou warnings de declaração:");
