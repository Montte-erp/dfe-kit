...
export const typeSafetyChecks: readonly Check[] = [
  {
    message: "Não use as casts; valide e converta pelo boundary de Schema/Effect.",
    test: ({ line }) => hasTypeAssertion(line),
    ignoreImportLine: true,
  },
  {
    message: "Evite qualquer tipo de tagged error apagado com any no catálogo de erro.",
    test: ({ line }) => hasTaggedErrorAny(line),
    ignoreImportLine: false,
  },
  {
    message: "Evite Any no efeito de domínio (`Effect.Effect<..., any, ...`).",
